<div class="px-4 sm:px-6 py-3 flex items-center justify-between shadow h-16 fixed top-0 left-0 right-0 z-50" style="background-color: #fff">
    <div class="flex items-center text-white">
    <img src="{{ asset('images/ceclogo.png') }}" alt="Icon" class="mr-2" style="height: 50px;">
    ...
    <div class="relative">
        @auth
            <div class="flex items-center cursor-pointer" id="opennavdropdown">
                ...
                <p class="text-sm text-black font-semibold leading-none" >{{ auth()->user()->name }}</p>
                <svg class="w-4 h-4 stroke-current text-black-200 ml-1 feather feather-chevron-down" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="6 9 12 15 18 9"></polyline></svg>
            </div>
            <div class="absolute top-0 right-0 mt-12 -mr-6 shadow rounded-bl rounded-br" style="background-color:#4169E1;">
                <div class="hidden h-24 w-48" id="navdropdown">
                    <div class="px-8 py-4 border-t border-blue-800">
                        <a href="{{ route('profile') }}" class="flex items-center pb-3 text-sm text-gray-200 font-semibold" style="text-decoration: none;">
        ...
        @else 
            <div class="flex items-center">
                @if (Route::has('about'))
                    <div>
                        <a class="flex items-center mr-4 text-sm text-gray-600 font-semibold" href="{{ route('about') }}" style="text-decoration: none;">
                            <span>About</span>
                        </a>
                    </div>
                @endif
